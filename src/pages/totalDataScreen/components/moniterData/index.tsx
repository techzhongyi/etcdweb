            ...
            <div className="right-second-content">
              <div className="pie-charts">
                {dataDetail.deliverable_assets_list_pie.length > 0 && (
                  <PieCharts pieData={dataDetail.deliverable_assets_list_pie} />
                )}
              </div>
                ...
                {dataDetail.deliverable_assets_list.map((item) => {
                  return (
                    <div className="progress-list">
                      ...
                      >
                        {item.type}({item.deliverable_count}/{item.heavy_count})
                      </div>
                      <div>
                        <Progress
                          percent={item.percentage * 100}
                        ...
                        <div className="progress-border">
                          <div
                            ...
                            style={{
                              width:
                                parseInt(
                                  (item.heavy_count / item.assets_count) * 100,
                                ) + '%',
                            }}
                          >
                            {item.heavy_count == 0 ? '' : item.heavy_count}
                          </div>
                          <div
                            className="qkCount"
                            style={{
                              width:
                                parseInt(
                                  (item.light_count / item.assets_count) * 100,
                                ) + '%',
                            }}
                          >
                            {item.light_count == 0 ? '' : item.light_count}
                          </div>
                          <div
                            className="vanCount"
                            style={{
                              width:
                                parseInt(
                                  (item.wan_count / item.assets_count) * 100,
                                ) + '%',
                            }}
                          >
                            {item.wan_count == 0 ? '' : item.wan_count}
                          </div>
                          <div
                            className="cxCount"
                            style={{
                              width:
                                parseInt(
                                  (item.carriage_count / item.assets_count) *
                                    100,
                                ) + '%',
                            }}
                          >
                            {item.carriage_count == 0
                              ? ''
                              : item.carriage_count}
                          </div>
                          <div
                            ...
                            style={{
                              width:
                                parseInt(
                                  (item.mounted_count / item.assets_count) *
                                    100,
                                ) + '%',
                            }}
                          >
                            {item.mounted_count == 0 ? '' : item.mounted_count}
                          </div>
                        </div>
                      </div>
                      {/* </Tooltip> */}
